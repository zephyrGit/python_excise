...
def save_to_txt():
    for i in range(1, 1001):
        url = 'http://m.maoyan.com/mmdb/comments/movie/248162.json?_v_=yes&offset=' + str(i)
        html = get_one_page(url)
        print('正在保存第%d页.' % i)
        for item in parse_ono_page(html):
            with open('欧洲攻略.txt', 'a', encoding='utf-8') as f:
                f.write(
...
if __name__ == '__main__':
        save_to_txt()
        delete_repeat(r'欧洲攻略.txt', r'欧洲攻略_new.txt')