...
f = open('欧洲攻略.txt', encoding='utf-8')
data = pd.read_csv(f, sep=',', header=None, encoding='utf-8', names=['data', 'nickName', 'city', 'rate', 'comment'])
...
geo = Geo("欧洲攻略", title_color="#fff", title_pos="center", width=1200,
...
geo.render('欧洲攻略.html')

comment = jieba.cut(str(data["comment"]), cut_all=False)
wl_space_split = " ".join(comment)
# 导入背景图
backgroud_Image = plt.imread('lcw.jpg')
...
wc = WordCloud(width=1024, height=768, background_color='white',
               mask=backgroud_Image, font_path='C:/Windows/Fonts/微软雅黑.ttf',
...
plt.show()
# 保存结果到本地
wc.to_file(r'lcw_wordcloud.jpg')
